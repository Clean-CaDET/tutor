...
public static class ProbePrompt
{
    public static string Build(ConceptRecord record)
    {
        ...
        sb.AppendLine("The <target> tag carries a level attribute (1-3) that shapes the question:");
        ...
        sb.AppendLine("- L3 — the learner has failed twice. Offer a sentence-completion scaffold: \"Dopuni rečenicu: '…zato što…'\" or \"Dovrši misao: …\". The blank must not contain the target text.");
        sb.AppendLine();
        ...
        sb.AppendLine("- If <soft-cap/> is present, signal that you'll wrap up once this is addressed.");
        sb.AppendLine();
        ...
        sb.AppendLine("The final user message contains: <target type=\"KeyProposition|KeyRelation\" key=\"…\" level=\"1-3\">…statement…</target>, optional <soft-cap/>, <instruction>…</instruction>.");
